...
void GetTemper_Init(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);					//将PA1引脚初始化为模拟输入，PA0被TIM2用于输出PWM
	...
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)&AD_Value;					//存储器基地址，给定存放AD转换结果的全局变量AD_Value
